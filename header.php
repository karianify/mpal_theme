...
<body <?php body_class($body_classes); ?>><div class="root">
	...
	$widgets_on_mobile_enabled = get_theme_mod('widgets_on_mobile_enabled');
	?>
	<header class="<?php echo esc_attr($header_type_class) ?><?php if($header_opacity_enabled == 0) echo ' header-opacity-enabled'; ?><?php if($sticky_disabled < 2) echo ' sticky-enabled'; ?><?php if($sticky_disabled == 0) echo " sticky-no-topbar"; ?><?php if($menu_animation_enabled == 0) echo " menu-animation-enabled"; ?><?php if($hover_delay_enabled == 0) echo " hover-delay-enabled"; ?><?php if($sticky_collapse == 0) echo " sticky-collapse"; ?><?php if($sticky_opacity_enabled == 0) echo " sticky-opacity-enabled"; ?><?php if(!$search_disabled) echo ' with-search-box'; ?><?php if (class_exists('Woocommerce')) { if(!$cart_disabled) {echo ' with-cart-box';} }?><?php if(!$widgets_on_mobile_enabled) echo ' lr-mi-with-widget-visible'; ?>"<?php if($sticky_trigger_position != '') {echo ' data-sticky-trigger-position="'.esc_attr($sticky_trigger_position).'"';}?><?php if($menu_animation_enabled == 0) { ?><?php if($data_menu_slidedown_duration != '') {echo ' data-menu-slidedown-duration="'.esc_attr($data_menu_slidedown_duration).'"';}?><?php if($data_menu_slideup_duration != '') {echo ' data-menu-slideup-duration="'.esc_attr($data_menu_slideup_duration).'"';}?><?php if($data_menu_fadein_duration != '') {echo ' data-menu-fadein-duration="'.esc_attr($data_menu_fadein_duration).'"';}?><?php if($data_menu_fadeout_duration != '') {echo ' data-menu-fadeout-duration="'.esc_attr($data_menu_fadeout_duration).'"';}?><?php } ?>>
		<?php if ($hide_top == 1) : ?>
			...
			<div>
				...
				<nav class="social social-light<?php if(!$social_colored_disabled) {echo ' social-colored';} ?>">
					<ul>
						...
						<li><a href="<?php echo esc_attr($link->url); ?>" class="<?php echo esc_attr($link->class) ?>" target="_blank"><?php echo esc_attr($link->name) ?></a></li>
						<?php endforeach; ?>
					</ul>
				</nav>
				<?php endif; ?>
				...
				<div class="box-social">
					<nav class="social<?php if(!$social_colored_disabled) {echo ' social-colored';} ?>">
						<ul>
							<?php $social_links = converio_get_social_links(); 
						foreach($social_links as $link) : ?>
							<li><a href="<?php echo esc_attr($link->url); ?>" class="<?php echo esc_attr($link->class) ?>" title="<?php echo esc_attr($link->name) ?>" target="_blank"><?php echo esc_attr($link->name) ?></a></li>
							<?php endforeach; ?>
						</ul>
				    </nav>
				</div>
